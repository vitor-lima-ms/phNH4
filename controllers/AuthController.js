...
module.exports = class AuthController {
    ...
    static async forgotPasswordPost(req, res) {
        ...
        if(!user) {
            req.flash("message", "Se o email estiver cadastrado, um link de redefinição será enviado!")
    ...
    static async resetPasswordGet(req, res) {
        ...
        if(!resetToken) {
            req.flash("message", "Token de redefinição inválido ou expirado. Por favor, solicite um novo link!")
    ...
    static async resetPasswordPost(req, res) {
        ...
        if(password1 !== password2) {
            req.flash("message", "As senhas não conferem. Tente novamente!")
            return res.render("auth/resetPassword", {token: token, tokenValid: true})
        }
        ...
        if(!resetToken || !user) {
            req.flash("message", "Token de redefinição inválido, expirado ou usuário não encontrado. Por favor, solicite um novo link!")
            return res.render("auth/resetPassword", {tokenValid: false})
        }
        ...
        try {
            ...
            req.flash("message", "Ocorreu um erro ao tentar redefinir sua senha. Tente novamente!")
            res.render("auth/resetPassword", {token: token, tokenValid: true})            
        }
    }
}